...
| 20 | in progress | Private-party marketing and organic acquisition; bounded persona and conversion refinement verified | Truthful availability from accepted slices | G1 |
| 21 | in progress | Charts, Room Recaps, Discover, and activity growth loop; bounded challenger-ladder refinement verified | Room reliability and privacy gates | G2 |
...
Proof-readiness checkpoint (2026-07-22): the Audience App now has a collapsed, on-demand Recent activity view backed by a user-only callable. It joins completed paid checkout evidence with sanitized server-recorded rewards and spends, exposes non-sensitive BeauRocks confirmation codes, and keeps the current Room Points balance visibly separate. Detailed Room-wide Points reconciliation remains in Host Advanced Diagnostics. This checkpoint does not approve the BeauBucks catalog or claim complete coverage of legacy client-side Points mutations.
...
Slice 10.1 completed the fail-closed Host/Audience authority contract: separate Points and BeauBucks balances, one `BeauBucks tonight` Host choice in internally authorized Rooms, paid-reaction authorization, collapsed Audience activity proof, and no public checkout.

Slice 10.2 adds a read-only paid-canary decision packet and a server-enforced checkout reservation. It derives a conservative spend/write envelope from the registered pack and reaction costs. Owner terms, operations, cohort, and activation decisions still block checkout, and public Terms/point-of-purchase copy remain a separately reviewed activation task.

Slice 10.3 changes only BeauBucks portability: BeauBucks now resolve to one signed-in BeauRocks account wallet across Rooms, while Points keep their existing name and Room participation behavior. A read-only production inventory found zero legacy BeauBucks wallet documents and zero BB to transfer, so the compatibility cutover has no customer-balance migration. Anonymous guests cannot buy or spend BeauBucks. Checkout remains disabled while the durable reaction-bank product, provider-neutral mobile grant path, terms, and paid cohort are completed.
...
Slice 05.4 is deployed to the controlled production cohort. The remaining identified YouTube metadata/playlist, Apple Music, Gemini, automatic trivia, and automatic/callable lyrics attempts now reserve and settle replay-safe operation documents under `youtube_metadata_lookup`, `apple_music_lookup`, or `ai_generation`. Host and Audience use one stable recovery vocabulary, background enrichment falls back instead of failing the Room, and Public TV remains deliberately independent of fresh variable-cost callables. Hosting release `1784709581207000` serves the client contract; the scoped Function revisions are recorded in `docs/USAGE_CONTROL_CONTRACT.md`. The owner production pass and evidence still keep Gate C2 open; no public pricing or postpaid collection behavior is enabled.

Slices 20-21 bounded refinement is verified for release. Public acquisition now leads with self-hosted karaoke at home and private parties, while Charts route Song Crowns, Singer Momentum, and Active Nights to their matching Host, singer, and venue paths. Real song projections carry a capped, sanitized top-three challenger ladder in the existing public document; moderation rebuilds it, opening scores remain presentation-only, and only evidence-backed results enter ItemList structured data. Quick Room creation now keeps BeauBucks out of the normal Points-economy choices until a Room is server-authorized, preserving the existing single in-Room cosmetics switch. Gates G1-G2 remain open for production review, content density, recap/listing integration, and broader release evidence.

Slices 20-21 bounded production release completed on 2026-07-22 from commit `bebd29f`. Functions revisions `logPerformance-00141-huh` and `moderatePublicChartResult-00007-jeg` plus Hosting release `1784770832827000` are live. Desktop and mobile production browser smoke passed Charts, all three metric tabs, the venue conversion state, and the Discover-to-Charts handoff without page errors or horizontal overflow. Gates G1-G2 remain open for content density, recap/listing integration, and broader acquisition evidence; this bounded release does not open paid Host checkout, BeauBucks checkout, unrestricted Host access, or offline claims.
