...
test('host app lazy-loads heavy host surfaces behind React.lazy boundaries', () => {
  ...
  assert.match(hostAppSource, /const ModerationInboxDrawer = React\.lazy\(\(\) => import\('\.\/components\/ModerationInboxDrawer'\)\);/);
...
test('host app wraps deferred host surfaces in suspense fallbacks', () => {
  ...
  assert.match(hostAppSource, /showModerationInbox \? \(\s*<React\.Suspense fallback=\{null\}>[\s\S]*<ModerationInboxDrawer/);
