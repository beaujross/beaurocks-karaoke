...
const AnimatedPoints = ({ value, onClick, className = '', rewardDelta = 0, rewardPulseKey = '', displayValue = null, caption = 'PTS' }) => {
    ...
    return (
        <button onClick={onClick} className={`relative overflow-visible bg-black/60 backdrop-blur-sm px-3 py-2 rounded-full border border-cyan-500/30 flex items-center gap-2 shadow-lg active:scale-95 transition-transform z-50 points-hint h-11 w-[120px] sm:w-[132px] justify-between ${showRewardBadge ? 'scale-[1.04] border-emerald-300/70 shadow-[0_0_28px_rgba(16,185,129,0.38)]' : ''} ${className}`}>
            <span className="text-cyan-300 font-black text-xl font-mono">{displayValue ?? display}</span>
            <span className="text-[11px] text-cyan-300 font-bold">{caption}</span>
...
const AudienceBrowseSongRow = ({
    ...
    <div className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900/70 p-3 hover:border-[#00C4D9]/40">
        ...
        <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-lg">
            <img src={art} alt={title} className="h-full w-full object-cover" />
            {loading ? (
                <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-black/60 text-[10px] uppercase tracking-[0.18em] text-zinc-200">
                    Load
                </div>
            ) : null}
        </div>
        <div className="min-w-0 flex-1">
            <div className="truncate text-[17px] font-black leading-tight text-white">{title}</div>
            <div className="truncate text-sm leading-tight text-zinc-400">{artist}</div>
            {ready ? (
                <div className="mt-1 inline-flex items-center gap-1 rounded-full border border-emerald-400/25 bg-emerald-500/12 px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.18em] text-emerald-200">
        ...
        <div className="ml-1 flex shrink-0 items-center gap-2">
            ...
            {typeof onTight15 === 'function' ? (
                <button
                    type="button"
                    onClick={onTight15}
                    aria-label="Add to Tight 15"
                    className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-amber-500/30 bg-amber-500/12 text-amber-300"
                >
                    <i className="fa-solid fa-list-ul text-[12px]"></i>
                </button>
            ) : null}
        </div>
    </div>
...
const getSafeGuitarLaneIndex = (laneIndex, laneCount = GUITAR_LANE_THEME.length) => {
    ...
    if (!Number.isFinite(numericLaneIndex)) return 0;
    return ((numericLaneIndex % normalizedLaneCount) + normalizedLaneCount) % normalizedLaneCount;
};
const getGuitarLaneTheme = (laneIndex) => GUITAR_LANE_THEME[getSafeGuitarLaneIndex(laneIndex)] || DEFAULT_GUITAR_LANE_THEME;
...
const SingerApp = ({ roomCode, uid }) => {
    ...
    const isVipAccount = vipUnlockPending
        ...
        || !!profile?.isVip
        || (Number(profile?.vipLevel || 0) > 0);
    const [songsTab, setSongsTab] = useState(initialAudienceDemoFixture?.songsTab || 'requests');
    const [socialTab, setSocialTab] = useState(initialAudienceDemoFixture?.socialTab || 'lounge'); // Sub-tab for Social
    const [profileSubTab, setProfileSubTab] = useState(initialAudienceDemoFixture?.profileSubTab || 'overview');
    ...
    const activeUid = useMemo(
        () => resolveAudienceSessionUid({
            ...
            authReadyUid: isQaAudienceFixture ? '' : authReadyUid,
            routeUid: uid
        }),
    ...
    const audienceReleaseChoiceDetails = useMemo(() => ({
        ...
        keepQueueMoving: String(audienceReleaseWindow?.choiceDetails?.keep_queue_moving || '').trim()
    }), [
    ...
    const audienceReleaseChoiceSongs = useMemo(() => ({
        ...
        keepQueueMoving: (songs || []).find((song) => song.id === String(audienceReleaseWindow?.choiceSongIds?.keep_queue_moving || '').trim()) || null
    }), [
        audienceReleaseWindow?.choiceSongIds?.keep_queue_moving,
        audienceReleaseWindow?.choiceSongIds?.slot_scene,
        songs
    ]);
    ...
    const audienceReleaseTone = useMemo(() => (
        isAudienceCoHostDecision
            ? {
                cardClass: 'border-amber-300/28 bg-[linear-gradient(145deg,rgba(35,20,10,0.96),rgba(24,16,12,0.92))] shadow-[0_20px_60px_rgba(120,53,15,0.26)]',
            ...
            : {
                ...
                badgeClass: 'border-white/10 bg-black/35 text-zinc-200',
    ...
    useEffect(() => {
        ...
        const handleMessage = (event) => {
            ...
            if (!payload || payload.type !== 'beaurocks-demo-fixture' || payload.surface !== 'audience') return;
            setDemoFixture(payload.fixture || null);
        };
        window.addEventListener('message', handleMessage);
        try {
            window.parent?.postMessage({ type: 'beaurocks-demo-ready', surface: 'audience' }, '*');
        } catch (_) {
            // Ignore parent postMessage issues in standalone app mode.
        }
    ...
    useEffect(() => {
        ...
        if (fixture.stagePanelCollapsedByTab && typeof fixture.stagePanelCollapsedByTab === 'object') {
            setStagePanelCollapsedByTab((prev) => ({ ...prev, ...fixture.stagePanelCollapsedByTab }));
        }
    ...
    const [reactionCooldownNowMs, setReactionCooldownNowMs] = useState(0);
    ...
    const roomBoostOffers = useMemo(() => {
        const picks = [];
        const seen = new Set();
        const pushIfRoomWide = (crate) => {
            if (!crate?.id || seen.has(crate.id)) return;
            const rewardScope = String(crate.rewardScope || 'room').toLowerCase();
            if (rewardScope === 'buyer') return;
            seen.add(crate.id);
            picks.push(crate);
        };
    ...
    const buildBrowseList = (list, listIdx) => {
        const enrichedSongs = decorateBrowseSongs(list.songs, { playableOnly: playableOnlyRequestMode });
        const songs = enrichedSongs.map((song, idx) => ({
            ...song,
    ...
    const browseCategories = BROWSE_CATEGORIES.map((list, idx) => buildBrowseList(list, idx)).filter((list) => list.songs.length > 0);
    const topicHits = TOPIC_HITS.map((list, idx) => buildBrowseList(list, idx + BROWSE_CATEGORIES.length)).filter((list) => list.songs.length > 0);
    const balladLights = [
    ...
    const takeoverLabel = getAudienceTakeoverLabel(takeoverKind);
    const takeoverOpen = isStreamlinedAudienceShell && !!takeoverKind && takeoverState === 'open';
    const takeoverMinimized = isStreamlinedAudienceShell && !!takeoverKind && takeoverState === 'minimized';

    const minimizeAudienceTakeover = useCallback(() => {
        if (!isStreamlinedAudienceShell || !takeoverKind) return;
        setTakeoverState('minimized');
        if (roomCode) {
            trackEvent('audience_takeover_minimized', {
                room_code: roomCode,
                shell_variant: shellVariant,
                takeover_kind: takeoverKind
            });
        }
    }, [isStreamlinedAudienceShell, roomCode, shellVariant, takeoverKind]);

    const reopenAudienceTakeover = useCallback(() => {
        if (!isStreamlinedAudienceShell || !takeoverKind) return;
        setTakeoverState('open');
        setTab('home');
        if (roomCode) {
            trackEvent('audience_takeover_reopened', {
                room_code: roomCode,
                shell_variant: shellVariant,
                takeover_kind: takeoverKind
            });
    ...
    useEffect(() => {
        if (!roomCode) return;
        const seenKey = `${roomCode}:${shellVariant}`;
        if (shellVariantSeenRef.current.has(seenKey)) return;
        shellVariantSeenRef.current.add(seenKey);
        trackEvent('audience_shell_variant_seen', {
            room_code: roomCode,
            shell_variant: shellVariant
        });
    }, [roomCode, shellVariant]);

    useEffect(() => {
        const previous = takeoverSeenRef.current;
        if (previous.kind && previous.kind !== takeoverKind && roomCode && isStreamlinedAudienceShell) {
            trackEvent('audience_takeover_completed', {
                room_code: roomCode,
                shell_variant: shellVariant,
                takeover_kind: previous.kind,
                final_state: previous.state || 'closed'
            });
        }

        if (!isStreamlinedAudienceShell || !takeoverKind) {
            takeoverSeenRef.current = { kind: takeoverKind, state: 'closed' };
            setTakeoverState('closed');
            return;
        }

        if (previous.kind !== takeoverKind) {
            setTakeoverState('open');
            setTab('home');
            if (roomCode) {
                trackEvent('audience_takeover_opened', {
                    room_code: roomCode,
                    shell_variant: shellVariant,
                    takeover_kind: takeoverKind
                });
        ...
        takeoverSeenRef.current = {
            kind: takeoverKind,
            state: previous.kind === takeoverKind ? previous.state : 'open'
        };
    }, [isStreamlinedAudienceShell, roomCode, shellVariant, takeoverKind]);
    ...
    const activeEventCredits = useMemo(() => {
        ...
        return {
            ...
            sourceProvider: String(source.sourceProvider || '').trim(),
            sourceCampaignCode: String(source.sourceCampaignCode || '').trim(),
            generalAdmissionPoints: Math.max(0, Number(source.generalAdmissionPoints || 0) || 0),
            vipBonusPoints: Math.max(0, Number(source.vipBonusPoints || 0) || 0),
            ...
            promoCampaigns: Array.isArray(source.promoCampaigns)
                ? source.promoCampaigns.map((campaign) => ({
                    ...
                    codeMode: String(campaign?.codeMode || '').trim(),
                    pointsReward: Math.max(0, Number(campaign?.pointsReward || 0) || 0),
                    safePerk: String(campaign?.safePerk || '').trim(),
                    maxRedemptions: Math.max(0, Number(campaign?.maxRedemptions || 0) || 0),
                    perUserLimit: Math.max(0, Number(campaign?.perUserLimit || 0) || 0),
                    requiresRoomJoin: campaign?.requiresRoomJoin !== false,
                    validFromMs: Math.max(0, Number(campaign?.validFromMs || 0) || 0),
                    validUntilMs: Math.max(0, Number(campaign?.validUntilMs || 0) || 0),
    ...
    const coHostFreeReactions = isRunOfShowCoHost
        && (
            ...
            || coHostCreditPolicy === CO_HOST_CREDIT_POLICIES.unlimited
        );
    const reactionTapCooldownMs = normalizeReactionTapCooldownMs(activeEventCredits.reactionTapCooldownMs ?? DEFAULT_REACTION_TAP_COOLDOWN_MS);
    const coHostReactionPolicyLabel = coHostUnlimitedCredits
        ? 'Unlimited co-host'
        : coHostFreeReactions
    ...
    const eventPromoSummary = useMemo(() => {
        ...
        return {
            promoCampaigns,
            usesGivebutter: activeEventCredits.sourceProvider === 'givebutter',
    ...
    const claimEventCredit = useCallback(async ({ campaignId = '', claimCode = '' } = {}) => {
        ...
        if (!roomCode || (!safeClaimCode && !safeCampaignId)) return null;
        setEventGrantBusy(true);
        try {
            ...
            if (result?.duplicate) {
                toast('That promo was already redeemed for this room.');
            } else if (grantedPoints > 0) {
                showRewardToast(`${result?.rewardLabel || 'Promo redeemed!'} +${grantedPoints} PTS`, grantedPoints, { durationMs: 3200 });
                setEventGrantCode('');
                if (result?.safePerk) {
                    toast(`Perk unlocked: ${String(result.safePerk).replaceAll('_', ' ')}`);
                }
            } else {
                toast(result?.rewardLabel || 'Promo redeemed.');
            }
            return result || null;
        } catch (error) {
            ...
            if (code.includes('permission-denied') || code.includes('not-found')) {
                toast('That promo is invalid, unavailable, or already gone.');
            } else if (code.includes('resource-exhausted')) {
                toast('That promo has already been fully claimed.');
            } else if (code.includes('failed-precondition')) {
                toast('That promo is not live for this room yet.');
            } else {
                toast('Could not redeem that promo right now.');
            }
            return null;
        } finally {
            setEventGrantBusy(false);
        }
    }, [roomCode, showRewardToast, toast]);

    useEffect(() => {
        if (typeof window === 'undefined' || !activeUid || !roomCode || !activeEventCredits.enabled) return;
        const params = new URLSearchParams(window.location.search);
        const claimCode = String(params.get('promoCode') || params.get('eventCode') || '').trim();
        const campaignId = String(params.get('promoCampaign') || params.get('eventGrant') || '').trim();
        if (!claimCode && !campaignId) return;
        const attemptKey = `${roomCode}:${activeUid}:${campaignId}:${claimCode}`;
        if (eventGrantAutoClaimRef.current === attemptKey) return;
        eventGrantAutoClaimRef.current = attemptKey;
        claimEventCredit({ campaignId, claimCode }).finally(() => {
            try {
                const nextUrl = new URL(window.location.href);
                nextUrl.searchParams.delete('eventGrant');
                nextUrl.searchParams.delete('eventCode');
                nextUrl.searchParams.delete('promoCampaign');
                nextUrl.searchParams.delete('promoCode');
                window.history.replaceState({}, document.title, nextUrl.toString());
            } catch {
                // Ignore URL cleanup failures.
            }
        });
    }, [activeEventCredits.enabled, activeUid, claimEventCredit, roomCode]);
    ...
    }, [roomCode, activeUid, getRoomUserProjection, user?.name, user?.avatar]);
    ...
    const flushStrobeBuffer = useCallback(async () => {
        ...
        try {
            try {
                ...
                console.warn('strobe reaction write failed', reactionError);
            }
            const sessionId = room?.strobeSessionId || Date.now();
            const isNewSession = user?.strobeSessionId !== sessionId;
            await mergeRoomUserVibeState({
                strobeSessionId: sessionId,
                strobeTaps: isNewSession
                    ? count
                    : Math.max(0, Number(user?.strobeTaps || 0)) + count,
                lastVibeAt: serverTimestamp()
            });
        } catch (e) {
            console.error(e);
    ...
    const flushStormLayerBuffer = useCallback(async () => {
        ...
        try {
            ...
            await Promise.all(layerEntries.map(async ([layer, count]) => {
                try {
                    await addDoc(
                    ...
                    console.warn('storm reaction write failed', layer, reactionError);
                }
            }));
...
const getEmojiChar = (t) => (EMOJI[t] || EMOJI.heart);
    ...
    const handleSelectAvatar = async (item, status) => {
        ...
        setAvatarUnlockModal({
            item,
            status,
            hint: getUnlockHint(item),
        });
        return;
    };
    ...
    const popTriviaRevealCorrectOption = popTriviaRevealCorrectIndex >= 0
        ...
        : '';
    const popTriviaRevealMyVote = showPopTriviaEndState && Number.isInteger(popTriviaRevealSnapshot?.myVote)
    ...
    useEffect(() => {
        ...
        if (tight15MigrationDoneRef.current === accountProfileUid) return;
        ...
        if (!tempList.length || savedList.length) {
            tight15MigrationDoneRef.current = accountProfileUid;
            return;
        }
        let cancelled = false;
        const migrate = async () => {
            try {
                ...
                await setDoc(doc(db, 'users', accountProfileUid), { tight15: merged }, { merge: true });
                await setDoc(
                    doc(db, 'artifacts', APP_ID, 'public', 'data', 'room_users', `${roomCode}_${activeUid}`),
                    { tight15Temp: merged },
                    { merge: true }
    ...
    }, [accountProfileUid, activeUid, isAnon, user, profile, roomCode, toast]);
    ...
    useEffect(() => {
        ...
        if (resolvedVip) {
            setVipUnlockPending(false);
        }
    }, [profile?.isVip, profile?.vipLevel, user?.isVip, user?.vipLevel, vipUnlockPending]);
    ...
    useEffect(() => {
        if (typeof window === 'undefined') return;
        try {
            localStorage.setItem('beaurocks_mobile_bottom_nav_compact', isBottomNavCollapsed ? '1' : '0');
        } catch {
    ...
    const triggerCooldownFlash = (reactionKey = '') => {
        setCooldownFlashKey(reactionKey);
        if (cooldownTimer.current) clearTimeout(cooldownTimer.current);
        cooldownTimer.current = setTimeout(() => {
            setCooldownFlashKey((current) => (current === reactionKey ? '' : current));
        }, 350);
    };
    useEffect(() => {
        const cooldownEntries = Object.values(reactionCooldownByType || {}).filter((value) => Number(value || 0) > Date.now());
        if (!cooldownEntries.length) {
            setReactionCooldownNowMs(0);
            return undefined;
        }
        const tick = () => setReactionCooldownNowMs(Date.now());
        tick();
        const interval = setInterval(tick, 100);
        return () => clearInterval(interval);
    }, [reactionCooldownByType]);
    useEffect(() => {
        const cooldownEntries = Object.values(coHostSignalCooldownById || {}).filter((value) => Number(value || 0) > Date.now());
        if (!cooldownEntries.length && !coHostSignalSheetOpen) {
            setCoHostSignalNowMs(0);
            return undefined;
        }
        const tick = () => setCoHostSignalNowMs(Date.now());
        tick();
        const interval = setInterval(tick, 500);
        return () => clearInterval(interval);
    }, [coHostSignalCooldownById, coHostSignalSheetOpen]);
    useEffect(() => {
    ...
    const canAffordRoomCost = useCallback((cost = 0) => {
        ...
        if (coHostUnlimitedCredits) return true;
    ...
    const normalizeAudienceYouTubeSearchItems = useCallback((rawItems = []) => (
        (Array.isArray(rawItems) ? rawItems : [])
            .map((item) => {
                ...
                return {
                    ...
                    artworkUrl100: String(item?.thumbnails?.medium?.url || item?.thumbnails?.default?.url || '').trim(),
    ...
    const getAudienceYouTubeResultMeta = useCallback((result = {}) => {
        ...
        if (playbackState.youtubePlaybackStatus === YOUTUBE_PLAYBACK_STATUSES.embeddable) {
            return {
                ...
                detail: 'Plays inside Public TV and stays in the normal show flow.',
            };
        }
        return {
    ...
    useEffect(() => {
        ...
        const raf = requestAnimationFrame(() => {
            try {
                manualRequestSongInputRef.current?.focus?.({ preventScroll: true });
            } catch {
    ...
    useEffect(() => {
        if (!isMarketingDemoEmbed || isMarketingDemoFixture || user || !roomCode || !activeUid) return;
    ...
    }, [activeUid, demoGuestProfile, isMarketingDemoEmbed, isMarketingDemoFixture, roomCode, user]);
    ...
    const uploadSelfieBlob = useCallback(async (blob, suffix = 'selfie', { uploadMode = 'direct' } = {}) => {
        ...
        const ensureRoomUserMembership = async (authUid) => {
            ...
            if (!existingRoomUserSnap.exists()) {
                await setDoc(roomUserRef, membershipSeed, { merge: true });
            } else {
    ...
    const resolveSelfieErrorMessage = (error, fallback = 'Selfie upload failed') => {
        ...
        if (code.includes('app-check') || message.includes('app check') || message.includes('recaptcha')) {
            return 'Security check expired. Keep the app open, wait a moment, and try again.';
        }
    ...
    const triggerLobbyPlayInteraction = async (interactionId) => {
        ...
        if (!key) return;
        const ultimateMeta = getVolleyOrbUltimate(key);
        const currentLobbyUid = getActiveUid();
        const previewParticipant = currentLobbyUid ? lobbyVolleyPreview?.participants?.[currentLobbyUid] : null;
        const ultimateCooldownRemainingMs = ultimateMeta
            ? Math.max(
                0,
                (Number(previewParticipant?.lastUltimateAtMs || 0) + VOLLEY_ORB_ULTIMATE_COOLDOWN_MS) - Date.now()
            )
            : 0;
        if (ultimateMeta && ultimateCooldownRemainingMs > 0) {
            const retryInSec = Math.max(1, Math.ceil(ultimateCooldownRemainingMs / 1000));
            if (Date.now() - lastLobbyPlayLimitToastAtRef.current > 1200) {
                toast(`${ultimateMeta.label} recharges in ${retryInSec}s.`);
        ...
        const ratePlan = getLobbyVolleyAudienceRatePlan(lobbyVolleyPreview || createLobbyVolleyState(), {
            strictMode,
            roomMaxPerMinute: room?.lobbyPlaygroundMaxPerMinute,
            roomPerUserCooldownMs: room?.lobbyPlaygroundPerUserCooldownMs,
            nowMs: now
        });
        const maxPerMinute = ratePlan.maxPerMinute;
        const perUserCooldownMs = ratePlan.perUserCooldownMs;
        ...
        markActive();
        if (navigator.vibrate) {
            try { navigator.vibrate(ultimateMeta ? [18, 30, 18] : 14); } catch {
                // Ignore vibration failures.
            }
        }
        try {
    ...
    const submitSong = async (s, a, art, options = {}) => { 
        ...
        try {
            ...
            if (playableOnlyRequestMode && !playbackReady) {
                toast('This room only accepts songs that already have approved playable backing. Try Browse or a different song.');
                return;
            }
            ...
            trackEvent('song_request', {
                room_code: roomCode,
                source: room?.bouncerMode ? 'bouncer' : 'standard',
                shell_variant: shellVariant
            });
    ...
    const updateProfile = async () => { 
        ...
        if (nameEmojiChanged && changeCost > 0 && !canAffordRoomCost(changeCost)) {
    ...
    const getAccountEmailErrorMessage = (error) => {
        const code = String(error?.code || '').toLowerCase();
        const message = String(error?.message || '').toLowerCase();
        if (code.includes('invalid-email')) return 'Enter a valid email address.';
        if (code.includes('missing-email')) return 'Enter your email to continue.';
        ...
        return 'Email verification failed. Send a fresh link and try again.';
    };

    const readStoredAudienceEmailLinkState = () => {
        if (typeof window === 'undefined') return null;
        try {
            const stored = JSON.parse(localStorage.getItem(AUDIENCE_EMAIL_LINK_STORAGE_KEY) || 'null');
    ...
    const completeAudienceAccountUpgrade = useCallback(async ({ verifiedUser, sourceUid, roomUserSeed = null, email, source = 'audience_email_verify' }) => {
        ...
        setProfile((prev) => ({
            ...(prev || {}),
            isVip: true,
            vipLevel: Math.max(nextVipLevel, Number(prev?.vipLevel || 0)),
        }));
        setUser((prev) => prev ? ({
            ...prev,
            isVip: true,
        ...
        if (typeof window !== 'undefined') {
            localStorage.removeItem(AUDIENCE_EMAIL_LINK_STORAGE_KEY);
        }
        setEmailLinkSent(false);
        setAccountEmail('');
    ...
    const sendAccountEmailLink = async () => {
        ...
        if (!normalizedEmail) {
            toast('Enter your email to continue.');
            return;
        }
        ...
        try {
            ...
            await sendBeauRocksEmailSignInLink({
                email: normalizedEmail,
                continueUrl: buildAudienceEmailLinkTarget(),
                roomCode,
                sourceUid
            });
            if (typeof window !== 'undefined') {
                localStorage.setItem(AUDIENCE_EMAIL_LINK_STORAGE_KEY, JSON.stringify({
                    email: normalizedEmail,
                    sourceUid,
                    roomCode,
                    sentAt: Date.now()
    ...
    useEffect(() => {
        if (typeof window === 'undefined') return;
        const emailLinkHref = window.location.href;
        if (!isSignInWithEmailLink(auth, emailLinkHref)) return;
        const storedState = readStoredAudienceEmailLinkState();
        const email = normalizeAccountEmail(accountEmail || storedState?.email || '');
        const attemptState = audienceEmailLinkAttemptRef.current || { href: '', phase: 'idle', email: '' };
        if (!email) {
            if (attemptState.href !== emailLinkHref || attemptState.phase !== 'needs_email') {
                audienceEmailLinkAttemptRef.current = { href: emailLinkHref, phase: 'needs_email', email: '' };
                setEmailLinkSent(false);
                setShowPhoneModal(true);
                toast('Open the verification link on the same device where you entered your email.');
            }
            return;
        }
        if (
            attemptState.href === emailLinkHref
            && (
                attemptState.phase === 'pending'
                || attemptState.phase === 'success'
                || (attemptState.phase === 'failed' && attemptState.email === email)
            )
        ) {
            return;
        }
        audienceEmailLinkAttemptRef.current = { href: emailLinkHref, phase: 'pending', email };
        let cancelled = false;
            ...
            try {
                ...
                try {
                    ...
                    if (canLinkCurrentAnonymousSession) {
                        ...
                        verifiedUser = signedIn?.user || auth.currentUser;
                    }
                } catch (linkError) {
                    const code = String(linkError?.code || '').toLowerCase();
                    const recoverableExistingAccountError = (
                        code.includes('credential-already-in-use')
                        || code.includes('email-already-in-use')
                        || code.includes('provider-already-linked')
                    );
                    if (!recoverableExistingAccountError) throw linkError;

                    const signedIn = await signInWithCredential(auth, credential);
                    verifiedUser = signedIn?.user || auth.currentUser;
                    const targetUid = String(verifiedUser?.uid || '').trim();
                ...
                if (!cancelled) {
                    audienceEmailLinkAttemptRef.current = { href: emailLinkHref, phase: 'success', email };
                    ...
                    stripEmailLinkParamsFromUrl();
                }
            } catch (error) {
                console.error('email link verify error', error);
                const errorCode = String(error?.code || '').toLowerCase();
                const invalidEmailLink = errorCode.includes('expired-action-code') || errorCode.includes('invalid-action-code');
                audienceEmailLinkAttemptRef.current = { href: emailLinkHref, phase: 'failed', email };
                if (!cancelled) {
    ...
    useEffect(() => {
        if (!roomCode) return;
        const keys = [];
        if (tab === 'social' && ['lounge', 'host'].includes(socialTab)) {
            keys.push(`chat:${socialTab}`);
        }
        if (tab === 'request' && songsTab === 'queue') {
            keys.push('queue');
        }
        ...
        keys.forEach((key) => {
            const seenKey = `${roomCode}:${shellVariant}:${key}`;
            if (audienceExperimentViewRef.current.has(seenKey)) return;
            audienceExperimentViewRef.current.add(seenKey);
            if (key.startsWith('chat:')) {
                trackEvent('audience_chat_opened', {
                    room_code: roomCode,
                    shell_variant: shellVariant,
                    chat_tab: key.split(':')[1] || 'lounge'
                });
                return;
            }
            if (key === 'queue') {
                trackEvent('audience_queue_viewed', {
                    room_code: roomCode,
                    shell_variant: shellVariant
                });
                return;
            ...
            if (key === 'tight15') {
                trackEvent('audience_tight15_opened', {
                    room_code: roomCode,
                    shell_variant: shellVariant
                });
                return;
            ...
            if (key === 'request_start') {
                trackEvent('audience_request_started', {
                    room_code: roomCode,
                    shell_variant: shellVariant
                });
            }
    ...
    if (room?.lightMode === 'guitar' && !(isStreamlinedAudienceShell && takeoverMinimized)) {
    ...
    if (room?.lightMode === 'strobe' && !(isStreamlinedAudienceShell && takeoverMinimized)) {
    ...
    if (isAudienceSelfieCam && !(isStreamlinedAudienceShell && takeoverMinimized)) {
        return (
            <div className="h-screen bg-black flex flex-col relative overflow-hidden">
                {renderStreamlinedTakeoverChrome('Selfie Cam')}
                <video ref={videoRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover"></video>
                {!cameraActive && (
    ...
    if (room?.activeMode
        ...
        const hideBingoOverlay = isBingo && (isStreamlinedAudienceShell ? takeoverMinimized : !showBingoOverlay);
        ...
        if (!hideBingoOverlay) {
            ...
            <div className="absolute inset-0">
                <GameContainer
                    ...
                    onClose={isStreamlinedAudienceShell ? minimizeAudienceTakeover : (isBingo ? () => setShowBingoOverlay(false) : undefined)}
                    closeLabel={isStreamlinedAudienceShell ? 'Minimize' : 'Close'}
    ...
    if (publicProfileOpen) {
        ...
        return (
            ...
            >
                <div
                    className="relative flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-[2rem] border border-cyan-400/25 bg-gradient-to-br from-[#1a1030] via-[#111827] to-[#0a0d12] shadow-[0_0_55px_rgba(0,196,217,0.22)] sm:max-w-lg sm:rounded-3xl"
                    onClick={(e) => e.stopPropagation()}
                >
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto custom-scrollbar touch-scroll-y px-4 py-4">
                        {publicProfileLoading ? (
                            ...
                            <>
                                <div className="bg-black/40 border border-cyan-300/20 rounded-2xl p-4 mb-4">
                                    <div className="text-[11px] uppercase tracking-[0.35em] text-zinc-400 mb-2">Fame Level</div>
                                    ...
                                    <div className="mt-2 text-sm text-zinc-300">{fame.total} fame points</div>
                                    {nextUnlock ? (
                                        <div className="mt-3 bg-black/30 border border-fuchsia-400/20 rounded-xl px-3 py-2">
                                            <div className="text-[11px] uppercase tracking-[0.28em] text-zinc-400">Next Unlock</div>
                                        ...
                                        <div className="mt-3 bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-xs text-zinc-300">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Location</div>
                                        <div className="text-white text-base font-bold">{data.vipProfile?.location || '-'}</div>
                                    </div>
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Birthday</div>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Performances</div>
                                        <div className="text-white font-black text-xl">{publicStats.performances || 0}</div>
                                    </div>
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Total Points</div>
                                        <div className="text-white font-black text-xl">{publicStats.totalPoints || 0}</div>
                                    </div>
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Loudest dB</div>
                                        <div className="text-white font-black text-xl">{publicStats.loudest || 0}</div>
                                    </div>
                                    <div className="bg-black/35 border border-white/10 rounded-xl p-3">
                                        <div className="text-[11px] text-zinc-400 uppercase tracking-[0.24em] mb-1">Emojis</div>
                                ...
                                <div className="bg-black/35 border border-white/10 rounded-2xl p-4 mb-4">
                                    <div className="text-[11px] uppercase tracking-[0.35em] text-zinc-400 mb-2">Top 3 From Tight 15</div>
                                    {topTight15.length === 0 ? (
                                ...
                                <div className="bg-black/35 border border-white/10 rounded-2xl p-4">
                                    <div className="text-[11px] uppercase tracking-[0.35em] text-zinc-400 mb-2">Performance Stats</div>
    ...
    if (avatarUnlockModal) {
        ...
        return (
            <div className="fixed inset-0 bg-black/80 z-[140] flex items-center justify-center p-6 text-white font-saira" onClick={() => setAvatarUnlockModal(null)}>
                <div className="w-full max-w-sm rounded-3xl border border-fuchsia-300/25 bg-gradient-to-br from-[#140d22] via-[#11131b] to-[#0a0d12] p-6 shadow-[0_0_40px_rgba(236,72,153,0.2)]" onClick={(e) => e.stopPropagation()}>
                    <div className="text-[10px] uppercase tracking-[0.35em] text-zinc-500">Avatar Unlock</div>
                    <div className="mt-3 flex items-center gap-4">
                        <div className="flex h-20 w-20 items-center justify-center rounded-3xl border border-white/10 bg-black/35 text-5xl">
                            {lockedAvatar.emoji || DEFAULT_EMOJI}
                        </div>
                        <div className="min-w-0">
                            <div className="text-2xl font-black text-white">{lockedAvatar.label || 'Locked Avatar'}</div>
                            <div className="mt-1 text-sm text-zinc-300">{lockedAvatar.flavor || avatarUnlockModal.hint}</div>
                    ...
                    <div className="mt-5 flex gap-2">
                        ...
                        {canBuyWithPoints ? (
                            ...
                            <button
                                onClick={() => setAvatarUnlockModal(null)}
                                className="flex-1 bg-[#00C4D9]/20 border border-[#00C4D9]/40 text-cyan-100 py-3 rounded-xl font-bold"
                            >
                                Got It
                            </button>
                        )}
    ...
    if (showPhoneModal) return (
        <div className="fixed inset-0 bg-black/80 z-[130] flex items-center justify-center p-6">
            <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(0,196,217,0.18),_rgba(24,24,27,0.96)_38%,_rgba(10,10,14,0.98)_100%)] p-6 shadow-[0_30px_80px_rgba(0,0,0,0.45)]">
                ...
                {allowsDonationAccess && roomSupportOffer && simplifyFestivalSupportAccess && (
                    <button
                        ...
                        className="mt-3 w-full rounded-2xl border border-emerald-300/25 bg-emerald-500/10 px-4 py-3 text-left text-emerald-100"
                    >
                        <div className="text-[11px] uppercase tracking-[0.24em] text-emerald-200">Optional support</div>
                        <div className="mt-1 text-base font-bold text-white">{supportCtaLabel}</div>
                        <div className="mt-1 text-sm text-emerald-100/80">
    ...
    if (showFameLevels) return (
        ...
        >
            <div
                className="relative flex max-h-[94dvh] w-full flex-col overflow-hidden rounded-t-[2rem] border border-cyan-300/35 bg-gradient-to-br from-[#1a1130] via-[#0f1726] to-[#0a0d12] shadow-[0_0_60px_rgba(34,211,238,0.22)] sm:max-w-xl sm:rounded-3xl"
                onClick={(e) => e.stopPropagation()}
            >
                ...
                <div className="sticky top-0 z-20 bg-black/45 backdrop-blur border-b border-white/10 px-5 py-4 flex items-start justify-between gap-3">
                    <div>
                        <div className="text-[11px] uppercase tracking-[0.34em] text-zinc-400">Fame System</div>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto custom-scrollbar touch-scroll-y px-5 py-4 space-y-3">
                    <div className="bg-black/40 border border-cyan-300/25 rounded-2xl p-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                <div className="text-[11px] uppercase tracking-[0.28em] text-zinc-400">Current Level</div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center justify-between gap-2">
                                                <div className="text-[11px] uppercase tracking-[0.28em] text-zinc-400">Level {level.level}</div>
                                                {isCurrent && <div className="text-[11px] uppercase tracking-[0.2em] text-cyan-200 bg-cyan-500/15 border border-cyan-400/30 px-2 py-0.5 rounded-full">Current</div>}
                                            </div>
                                            <div className="text-base font-bold text-white">{level.name}</div>
                                            <div className="text-sm text-zinc-300">Unlocks at {level.minFame} FP - Next {nextLabel}</div>
                                        </div>
                                    </div>
                                    <div className="mt-3 grid gap-2">
                                        <div className="text-sm text-zinc-200">
    ...
    const lobbyRatePlan = getLobbyVolleyAudienceRatePlan(lobbyVolleyPreview || createLobbyVolleyState(), {
        strictMode: lobbyPlayStrictMode,
        roomMaxPerMinute: room?.lobbyPlaygroundMaxPerMinute,
        roomPerUserCooldownMs: room?.lobbyPlaygroundPerUserCooldownMs,
        nowMs: lobbyVolleyNowMs
    });
    ...
    const homeStageInteractionState = showPopTriviaCard
        ...
        : lobbyVolleySceneActive
            ? 'volley'
            : canSeeAudienceReleaseWindow && isAudienceSongFaceOffDecision
                ? ''
    ...
    const autoCompactEmptyStageCard = shouldOfferCompactEmptyStageCard && (lobbyVolleySceneActive || !stageHomePanelExpanded);
    const showCompactEmptyStageCard = !currentSinger && (stagePanelCollapsed || autoCompactEmptyStageCard);
    ...
    const compactHomeStageBadgeLabel = homeStageInteractionState === 'trivia'
        ? (
            popTriviaQuestion
    ...
    const compactHomeStageBadgeIcon = homeStageInteractionState === 'trivia'
        ...
        : homeStageInteractionState === 'volley'
            ? 'fa-bolt'
            : 'fa-check-to-slot';
    const compactHomeStageBadgeClass = homeStageInteractionState === 'trivia'
        ? 'border-cyan-300/45 bg-cyan-500/16 text-cyan-50 shadow-[0_0_18px_rgba(34,211,238,0.25)]'
        : homeStageInteractionState === 'volley'
            ? 'border-fuchsia-300/45 bg-fuchsia-500/16 text-fuchsia-50 shadow-[0_0_18px_rgba(217,70,239,0.22)]'
    ...
    const compactStageBadgeLabel = homeStageInteractionState
        ...
        : activePrimaryStageTab === 'request'
            ...
            : activePrimaryStageTab === 'social'
                ? 'Social Below'
                : 'Expand Stage';
    const compactStageBadgeIcon = homeStageInteractionState
        ? compactHomeStageBadgeIcon
        : activePrimaryStageTab === 'request'
            ? 'fa-music'
            : activePrimaryStageTab === 'social'
                ? 'fa-comments'
                : 'fa-up-right-and-down-left-from-center';
    const compactStageBadgeClass = homeStageInteractionState
        ? compactHomeStageBadgeClass
        : 'border-cyan-300/35 bg-cyan-500/14 text-cyan-50 shadow-[0_0_18px_rgba(34,211,238,0.18)]';
    const compactEmptyStageModeIcon = lobbyVolleySceneActive
        ? 'fa-bolt'
        : activePrimaryStageTab === 'request'
            ? 'fa-music'
            : activePrimaryStageTab === 'social'
                ? 'fa-comments'
                : 'fa-table-cells';
    const compactEmptyStageModeLabel = lobbyVolleySceneActive
        ? 'Controls Below'
        : `${activePrimaryStageTabLabel} Below`;
    const compactEmptyStageTitle = lobbyVolleySceneActive
        ? 'Stage collapsed for volley mode'
        : stagePanelCollapsed
            ? 'Stage collapsed for more room'
            : 'Stage is empty';
    const compactEmptyStageDetail = lobbyVolleySceneActive
        ? 'Use the volley buttons below without the stage taking over the screen.'
        : stagePanelCollapsed
            ? `${activePrimaryStageTabLabel} content is below. Expand anytime to see the full stage card.`
            : 'Find a song or join the queue.';
    const compactEmptyStageMetaLabel = lobbyVolleySceneActive
        ? 'Mode'
        : stagePanelCollapsed
            ? 'View'
            : 'Room';
    const compactEmptyStageMetaValue = lobbyVolleySceneActive
        ? 'VOLLEY'
        : stagePanelCollapsed
            ? activePrimaryStageTabLabel.toUpperCase()
            : roomCode;
    ...
    const sendCoHostQuickSignal = async (signalId = '') => {
        const meta = getCoHostSignalMeta(signalId);
        if (!meta || !roomCode || !canUseCoHostQuickSignals) return;
        const now = Date.now();
        const cooldownUntil = Number(coHostSignalCooldownById?.[meta.id] || 0);
        if (cooldownUntil > now) {
            const waitSec = Math.max(1, Math.ceil((cooldownUntil - now) / 1000));
            toast(`Wait ${waitSec}s before sending ${meta.shortLabel.toLowerCase()} again.`);
            return;
        }
        try {
            ...
            setCoHostSignalSheetOpen(false);
            toast(meta.toastLabel || 'Signal sent to host.');
        } catch (error) {
        ...
        >
                    ...
                    <div className={`mx-auto max-w-[26rem] rounded-[1.35rem] border px-3.5 py-3.5 backdrop-blur ${audienceReleaseTone.cardClass}`}>
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                ...
                                {isAudienceCoHostDecision ? (
                                    <div className={`mt-1.5 inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.15em] ${audienceReleaseTone.pillClass}`}>
                                ...
                                {isAudienceSongFaceOffDecision ? (
                                    ...
                                    <div className={`mt-1.5 text-[11px] ${audienceReleaseTone.helperClass}`}>
                                        Pick who should fill this slot. Host confirms.
                                    </div>
                                ) : isAudienceCoHostDecision ? (
                                    <div className={`mt-1.5 text-[11px] ${audienceReleaseTone.helperClass}`}>
                                        Co-hosts are helping steer what happens next.
                                    </div>
                                ) : audienceReleaseWindow?.itemTitle ? (
                                    <div className="mt-1.5 text-[11px] text-zinc-400">
                                        {String(audienceReleaseWindow.itemTitle).trim()}
                                    </div>
                                ) : null}
                            </div>
            ...
            {canUseCoHostQuickSignals ? (
                <div
                    ...
                    data-feature-id="cohost-quick-signals"
                >
                    <div className="pointer-events-auto mx-auto flex max-w-[26rem] flex-col items-end gap-2">
                        {coHostSignalSheetOpen ? (
                            <div className="w-full rounded-[1.25rem] border border-amber-300/18 bg-[linear-gradient(145deg,rgba(24,17,10,0.97),rgba(11,10,18,0.94))] px-3 py-3 shadow-[0_18px_48px_rgba(0,0,0,0.34)]">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0 flex items-start gap-3">
                                        <div className="h-14 w-14 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-lg">
                                            {currentPerformanceSignalContext.artworkUrl ? (
                                                <img src={currentPerformanceSignalContext.artworkUrl} alt={currentPerformanceSignalContext.songTitle || 'Current performance'} className="h-full w-full object-cover" />
                                            ) : (
                                                <div className="flex h-full w-full items-center justify-center text-lg text-amber-200">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="mt-0.5 text-[11px] text-zinc-300">
                                                {currentPerformanceSignalContext.singerName || currentPerformanceSignalContext.artistName
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setCoHostSignalSheetOpen(false)}
                                        className="inline-flex min-h-[34px] items-center justify-center rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-zinc-100"
                                    >
                                        <i className="fa-solid fa-xmark"></i>
                                    </button>
                                </div>
                                <div className="mt-3 grid grid-cols-2 gap-2">
                                    {audioCoHostSignals.map((signal) => {
                                        ...
                                        const remainingSec = Math.max(1, Math.ceil((cooldownUntil - coHostSignalNowMs) / 1000));
                                        return (
                                            <button
                                                key={signal.id}
                                                type="button"
                                                disabled={coolingDown}
                                                onClick={() => sendCoHostQuickSignal(signal.id)}
                                                className={`rounded-2xl border px-3 py-2.5 text-left transition ${
                                                    coolingDown
                                                        ? 'cursor-not-allowed border-white/10 bg-black/25 text-zinc-500'
                                                        : 'border-amber-300/22 bg-black/28 text-white hover:border-amber-200/45 hover:bg-amber-500/10'
                                                }`}
                                            >
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className={`inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em] ${coolingDown ? 'text-zinc-500' : 'text-white'}`}>
                                                        <i className={`fa-solid ${signal.icon} ${coolingDown ? 'text-zinc-500' : 'text-amber-200'}`}></i>
                                                        {signal.shortLabel}
                                                    </span>
                        ...
                        <button
                            type="button"
                            onClick={() => setCoHostSignalSheetOpen((prev) => !prev)}
            ...
            {showBallad && !(isStreamlinedAudienceShell && takeoverMinimized) && (
            ...
            {showBanger && !(isStreamlinedAudienceShell && takeoverMinimized) && (
                <div className="absolute inset-0 z-[80] pointer-events-none overflow-hidden">
                    <div className="absolute inset-0 vibe-banger"></div>
                    <div className="absolute inset-0 bg-gradient-to-b from-pink-500/25 via-transparent to-cyan-500/25"></div>
                    <div className="absolute top-24 left-1/2 -translate-x-1/2 text-[10px] font-bold tracking-[0.45em] text-white/80 uppercase">Banger Mode - Feel The Bass</div>
                    ...
                    {currentSinger ? (
                        <div data-feature-id="singer-current-performance-card" className={`bg-indigo-900/80 rounded-2xl border border-indigo-500/30 shadow-lg backdrop-blur-md relative overflow-hidden ${isNativeMobileLayout ? 'mobile-native-stage-card' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-indigo-200">
                                                ...
                                                <span className={`inline-flex max-w-full items-center gap-1.5 whitespace-nowrap rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${compactStageBadgeClass}`}>
                                                    <i className={`fa-solid ${compactStageBadgeIcon} text-[9px]`}></i>
                                                    <span className="truncate">{compactStageBadgeLabel}</span>
                                                </span>
                                            </div>
                            ...
                            <div className={`${showCompactHomeStageCard ? 'hidden ' : ''}px-4 py-3 border-b border-white/10 bg-gradient-to-r from-black/70 via-black/30 to-black/70 ${isNativeMobileLayout ? 'mobile-native-stage-meta' : ''}`}>
                                ...
                                <div className="mt-3 flex flex-wrap items-center gap-2">
                                    {!isStreamlinedAudienceShell && room?.activeMode === 'bingo' && !showBingoOverlay && room?.bingoAudienceReopenEnabled !== false && (
                                        <button
                                    ...
                                    {showCompactHomeStageCard && (
                                        <span className={`inline-flex max-w-full items-center gap-1.5 whitespace-nowrap rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${compactStageBadgeClass}`}>
                                            <i className={`fa-solid ${compactStageBadgeIcon} text-[9px]`}></i>
                                            <span className="truncate">{compactStageBadgeLabel}</span>
                                        </span>
                                    )}
                                    ...
                                    <div data-feature-id="pop-trivia-card" className="mt-3 overflow-hidden rounded-3xl border-2 border-yellow-300/70 bg-gradient-to-br from-[#070b1a]/98 via-[#11162b]/98 to-[#180a1f]/98 shadow-[0_18px_54px_rgba(250,204,21,0.28)] backdrop-blur p-4">
                                        ...
                                        {popTriviaQuestion ? (
                                            <>
                                                ...
                                                <div className="mt-3 max-h-[min(42dvh,25rem)] overflow-y-auto overscroll-contain pr-1 touch-scroll-y custom-scrollbar">
                                                    <div className="text-base font-black text-white leading-snug">{popTriviaQuestion.q}</div>
                                                    <div className="mt-2 text-[11px] uppercase tracking-[0.2em] text-cyan-100/80">
                        ...
                        <div className="bg-zinc-800 rounded-2xl border border-dashed border-zinc-600 text-center text-zinc-400 transition-all overflow-hidden">
                            {showCompactEmptyStageCard && (
                                <button
                                    type="button"
                                    disabled={lobbyVolleySceneActive}
                                    onClick={() => {
                                        if (lobbyVolleySceneActive) return;
                                        pulseNativeUiFeedback();
                                        expandActiveStagePanel();
                                    }}
                                    className={`w-full px-4 py-3 text-left border-b border-white/10 bg-gradient-to-r from-black/70 via-black/30 to-black/70 ${lobbyVolleySceneActive ? 'cursor-default' : ''}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-12 h-12 rounded-xl bg-indigo-700/40 flex items-center justify-center text-2xl shadow-md flex-shrink-0">
                                            {lobbyVolleySceneActive ? String.fromCodePoint(0x26A1) : String.fromCodePoint(0x1F3A4)}
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-indigo-200">
                                                <span>{lobbyVolleySceneActive ? 'Volley orb live' : 'Stage ready'}</span>
                                                <span className="inline-flex max-w-full items-center gap-1.5 whitespace-nowrap rounded-full border border-pink-300/35 bg-pink-500/14 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-pink-50 shadow-[0_0_18px_rgba(236,72,153,0.22)]">
                                                    <i className={`fa-solid ${compactEmptyStageModeIcon} text-[9px]`}></i>
                                                    <span className="truncate">{compactEmptyStageModeLabel}</span>
                                                </span>
                                            </div>
                                            <div className="mt-1 text-base font-black leading-tight text-white">
                                                {compactEmptyStageTitle}
                                            </div>
                                            <div className="text-sm text-indigo-100/85">
                                                {compactEmptyStageDetail}
                                            </div>
                                        </div>
                                        <div className="shrink-0 text-right">
                                            <div className="text-[10px] uppercase tracking-[0.22em] text-zinc-400">{compactEmptyStageMetaLabel}</div>
                                            <div className="font-bebas text-xl tracking-[0.2em] text-cyan-200">{compactEmptyStageMetaValue}</div>
                                            <div className="mt-1 text-[11px] text-zinc-300">{compactEmptyStageMetaHint}</div>
                                        </div>
                                    </div>
                                </button>
                            )}
                            <div className={`${showCompactEmptyStageCard ? 'hidden ' : ''}px-4 py-3 border-b border-white/10 bg-gradient-to-r from-black/70 via-black/30 to-black/70`}>
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <div className="flex min-w-0 shrink-0 flex-wrap items-center justify-end gap-2">
                                        ...
                                        <button onClick={copyInviteLink} className="flex items-center gap-1 text-base font-bold text-white/85 bg-black/40 border border-white/10 px-3 py-1.5 rounded-full leading-none hover:bg-white/10">
                                            <i className="fa-solid fa-link stage-icon text-white/70"></i>
                                            Share
                                        </button>
                                    </div>
                            ...
                            <div className={`${showCompactEmptyStageCard ? 'hidden ' : ''}p-4 border-t border-white/10`}>
                                <div className="text-2xl mb-1 text-white">{String.fromCodePoint(0x1F3A4)} Stage is Empty</div>
                                <div className="text-sm text-zinc-300">Use the extra space to browse, then come back here when the next singer goes live.</div>
                                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    ...
                                    <button
                                        ...
                                        className="rounded-2xl border border-cyan-400/30 bg-cyan-500/12 px-4 py-3 text-sm font-black uppercase tracking-[0.16em] text-cyan-100 hover:bg-cyan-500/22"
                                    >
                                        View Queue
                                    </button>
                                </div>
                            ...
                            <div className="rounded-2xl border border-cyan-300/50 bg-gradient-to-br from-cyan-500/16 via-[#0a1020] to-fuchsia-500/16 p-3 space-y-3 shadow-[0_0_26px_rgba(34,211,238,0.24)]">
                                <div className="flex items-center justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <div className="text-lg font-black text-white leading-tight">{lobbyMobileMainLine}</div>
                                    </div>
                    ...
                    <div className="flex flex-col h-full">
                            ...
                            <React.Fragment>
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        paddingLeft: mobileSideInsetLeft,
                                        paddingRight: mobileSideInsetRight,
                                        paddingBottom: `calc(${mobileSafeBottomInset} + 12px)`
                                    }}
                                >
                                    <div className="flex min-h-0 flex-1 flex-col rounded-[1.75rem] border border-cyan-300/25 bg-[#090d18]/96 shadow-[0_22px_60px_rgba(0,0,0,0.45)]">
                                    <div className="flex items-center justify-between gap-3 px-4 pt-4 pb-3 border-b border-white/10">
                                        <div>
                                            <div className="text-xs uppercase tracking-[0.3em] text-cyan-200/80">
                                                {catalogSearchMode === 'youtube' ? 'YouTube Search' : 'Song Search'}
                                            </div>
                                    ...
                                    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar touch-scroll-y px-3 py-3">
                                        {searchQ.trim().length < 3 ? (
                                            ...
                                            <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 px-4 py-5 text-sm text-zinc-300">
                                                No matches yet. Try a more specific song + artist search.
                                            </div>
                                        )}
                                    </div>
                                    {catalogSearchMode === 'catalog' && backingChoiceState.open && (
                        ...
                        {manualRequestComposerOpen && (songsTab === 'requests' || (isStreamlinedAudienceShell && songsTab === 'browse')) && renderAudienceViewportSheet(
                            <React.Fragment>
                                ...
                                >
                                    <div className="flex min-h-0 flex-1 flex-col rounded-[1.75rem] border border-pink-300/25 bg-[#090d18]/97 shadow-[0_22px_60px_rgba(0,0,0,0.45)]">
                                    <div className="flex items-center justify-between gap-3 px-4 pt-4 pb-3 border-b border-white/10">
                                        <div className="text-left">
                                    ...
                                    >
                                        ...
                                        <label className="flex items-center gap-3 rounded-2xl border border-pink-300/18 bg-pink-500/8 px-4 py-3 text-left">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-sm text-zinc-300">Let the host pair you if someone else wants the same song.</div>
                                            </div>
                                        </label>
                                                    ...
                                                    <AudienceBrowseSongRow
                                                        ...
                                                        onRequest={async () => {
                                                            ...
                                                            submitSong(song.title, song.artist, art || song.art, song.backing?.mediaUrl ? {
                                                                mediaUrl: song.backing.mediaUrl,
            ...
            {takeoverMinimized && takeoverKind && (
                <button
                    onClick={() => {
                        pulseNativeUiFeedback();
                        reopenAudienceTakeover();
                    }}
            ...
            {!takeoverOpen && !isStreamlinedAudienceShell && (
                ...
                >
                    ...
                    <div
                        className={`relative py-1 flex ${isNativeMobileLayout ? 'mobile-native-tabbar-row' : ''}`}
                        style={{
                    ...
                    >
                        <button
                            onClick={() => { pulseNativeUiFeedback(); setTab('home'); }}
                            className={`flex-1 py-2 flex flex-col items-center gap-1 leading-tight ${isNativeMobileLayout ? 'mobile-native-tab-btn' : ''} ${(tab === 'home' || (isStreamlinedAudienceShell && tab === 'social')) ? `${isNativeMobileLayout ? 'mobile-native-tab-btn-active' : ''} text-[#FF7AC8] drop-shadow-[0_0_12px_rgba(255,122,200,0.6)]` : 'text-zinc-300'}`}
                        >
                            <i className="fa-solid fa-champagne-glasses text-[24px]"></i>
                            <span className="text-[13px] font-semibold">PARTY</span>
                        </button>
                        <button
                            data-feature-id="singer-nav-songs"
                            onClick={() => { pulseNativeUiFeedback(); setTab('request'); }}
                            className={`flex-1 py-2 flex flex-col items-center gap-1 leading-tight ${isNativeMobileLayout ? 'mobile-native-tab-btn' : ''} ${tab === 'request' ? `${isNativeMobileLayout ? 'mobile-native-tab-btn-active' : ''} text-[#46D7E8] drop-shadow-[0_0_12px_rgba(70,215,232,0.55)]` : 'text-zinc-300'}`}
                        >
                            <i className="fa-solid fa-music text-[24px]"></i>
                            <span className="text-[13px] font-semibold">SONGS</span>
                        </button>
                        {!isStreamlinedAudienceShell && (
                            <button
                                onClick={() => {
                                ...
                                className={`flex-1 py-2 flex flex-col items-center gap-1 leading-tight ${isNativeMobileLayout ? 'mobile-native-tab-btn' : ''} ${tab==='social' ? `${isNativeMobileLayout ? 'mobile-native-tab-btn-active' : ''} text-[#FF7AC8] drop-shadow-[0_0_12px_rgba(255,122,200,0.6)]` : 'text-zinc-300'} relative`}
                            >
                                <i className="fa-solid fa-comments text-[24px]"></i>
                                {chatUnread && <span className="absolute top-2 right-8 h-2.5 w-2.5 rounded-full bg-pink-400 ring-2 ring-pink-300/60 shadow-[0_0_10px_rgba(255,103,182,0.8)]"></span>}
