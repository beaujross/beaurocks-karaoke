...
const SupportContextCard = ({ context }) => {
  if (!context || context.source !== 'host_panel_feedback') return null;
  const location = String(context.workspaceSection || context.tab || '').replace(/[._-]+/g, ' ');
  const performance = [context.performanceTitle, context.performanceArtist].filter(Boolean).join(' · ');
  const captured = Number(context.capturedAtMs || 0);
  return <div className="border-b border-white/10 bg-cyan-500/[0.04] px-4 py-3" data-host-support-context>
    <div className="flex flex-wrap items-center justify-between gap-2"><Eyebrow tone="pink">Captured Host Panel context</Eyebrow>{captured ? <span className="text-[10px] text-zinc-500">{new Date(captured).toLocaleString()}</span> : null}</div>
    <div className="mt-2 flex flex-wrap gap-1.5">
      {context.roomCode ? <span className="rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-[10px] font-bold text-zinc-300">Room {context.roomCode}{context.roomName ? ` · ${context.roomName}` : ''}</span> : null}
      {location ? <span className="rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-[10px] font-bold capitalize text-zinc-300">{location}</span> : null}
      <span className="rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-[10px] font-bold text-zinc-300">{Number(context.queueCount || 0)} queued</span>
      <span className="rounded-full border border-white/10 bg-black/25 px-2.5 py-1 text-[10px] font-bold text-zinc-300">{performance ? `Live: ${performance}` : 'No performance live'}</span>
    </div>
  </div>;
};
      ...
      <div className="relative z-10 flex h-[calc(100vh-76px)] min-h-0 flex-col md:flex-row">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-3 custom-scrollbar sm:p-4 lg:p-5">
          <div className="mx-auto max-w-[1500px]">
            ...
            {isOps && tab === 'hosts' ? <HostRoster summary={summary} selectedHostId={selectedHostId} onSelectHost={openHost} onCloseHost={closeHost} onChanged={async () => setSummary(await getHostLifecycleReportingSummary({}))} /> : null}
