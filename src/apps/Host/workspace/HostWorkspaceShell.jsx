...
const HostWorkspaceShell = ({ context, children, showContext = true, fullBleed = false }) => {
    return (
        <div className={`flex-1 min-h-0 bg-zinc-950 overflow-hidden ${fullBleed ? '' : 'border border-zinc-800 rounded-xl'}`}>
            <div className={`h-full min-h-0 grid ${showContext ? 'grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px]' : 'grid-cols-1'}`}>
