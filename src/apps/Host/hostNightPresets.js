...
const normalizePresetSettings = (value = {}, fallback = {}) => ({
    ...
    oneMinuteMicEnabled: value?.oneMinuteMicEnabled !== undefined ? !!value.oneMinuteMicEnabled : !!fallback.oneMinuteMicEnabled,
    performanceProgressionMode: String(value?.performanceProgressionMode || fallback?.performanceProgressionMode || 'full_song').trim().toLowerCase() === 'one_minute_mic' ? 'one_minute_mic' : 'full_song',
    oneMinuteMicOpeningWindowSec: clampNumber(value?.oneMinuteMicOpeningWindowSec ?? fallback?.oneMinuteMicOpeningWindowSec ?? 60, 15, 180, 60),
    oneMinuteMicVoteWindowSec: clampNumber(value?.oneMinuteMicVoteWindowSec ?? fallback?.oneMinuteMicVoteWindowSec ?? 12, 5, 45, 12),
    ...
    hypeMeterDisplayMode: String(value?.hypeMeterDisplayMode || fallback?.hypeMeterDisplayMode || 'score_integrated').trim() || 'score_integrated',
    ...
    hideNonEmbeddableYouTube: value?.hideNonEmbeddableYouTube !== undefined ? value.hideNonEmbeddableYouTube !== false : fallback?.hideNonEmbeddableYouTube !== false,
