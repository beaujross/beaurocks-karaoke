...
const hostTab = (id = "") => id === "join_identity" ? "Lobby" : id === "trivia_break" ? "Games" : id === "karaoke_launch" ? "Browse" : "Stage";
...
export const DemoHostRoomShell = ({ roomCode, activeScene, hostTypedSearch, hostResults, queueSnapshot, hostControlProgress, hostFocusFrame, hostCursorStyle, tapCoach }) => (
  <div className="relative h-full overflow-hidden rounded-[24px] bg-[#050914] text-white">
    ...
    <div className="relative flex h-full flex-col">
      ...
      <div className="grid flex-1 grid-cols-[1.24fr_0.92fr] gap-3 p-4">
        <div className="flex min-h-0 flex-col gap-3">
          <div className="rounded-[24px] border border-indigo-400/25 bg-gradient-to-br from-indigo-900/70 via-[#10172a] to-[#070d18] p-4 shadow-[0_18px_44px_rgba(0,0,0,0.32)]">
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[11px] uppercase tracking-[0.26em] text-indigo-200">Now performing</div><div className="mt-2 truncate text-2xl font-black text-white">{activeScene.singer.name}</div><div className="mt-1 truncate text-sm italic text-indigo-100/85">{activeScene.tv.title}</div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full border border-cyan-400/40 bg-cyan-500/10 px-3 py-1 text-[10px] uppercase tracking-[0.16em] text-cyan-100">{activeScene.host.status}</span><span className="rounded-full border border-white/10 bg-black/25 px-3 py-1 text-[10px] uppercase tracking-[0.16em] text-zinc-200">{activeScene.singer.prompt}</span></div></div><div className="flex h-16 w-16 items-center justify-center rounded-[22px] border border-amber-300/35 bg-amber-500/18 text-amber-100 shadow-md"><i className={`fa-solid ${singerIcon(activeScene.singer.emoji)} text-xl`} /></div></div>
            <div className="mt-4 grid grid-cols-3 gap-3"><Metric label="Audience" value={activeScene.audience.metricValue} /><Metric label="Mode" value={activeScene.tv.mode} accent="amber" /><Metric label="Next move" value={activeScene.host.actionLabel} accent="pink" /></div>
          </div>
          <div className="rounded-[24px] border border-white/10 bg-black/35 p-4">
            <div className="flex items-center justify-between gap-3"><div className="text-[11px] uppercase tracking-[0.22em] text-zinc-400">{activeScene.host.search ? "Search + queue" : "Live controls"}</div><div className="text-[10px] uppercase tracking-[0.18em] text-cyan-100">{activeScene.host.actionLabel}</div></div>
          ...
          <div className="min-h-0 rounded-[24px] border border-white/10 bg-black/30 p-4"><div className="flex items-center justify-between gap-3"><div className="text-[11px] uppercase tracking-[0.22em] text-zinc-400">Incoming room activity</div><div className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">Live</div></div><div className="mt-3 grid gap-2">{activeScene.audience.feed.map((item, index) => <div key={`${activeScene.id}_${item}`} className={`rounded-2xl border px-3 py-3 ${index === 0 ? "border-emerald-300/30 bg-emerald-500/10" : "border-white/10 bg-white/5"}`}><div className="text-[10px] uppercase tracking-[0.16em] text-zinc-400">{index === 0 ? "Latest room update" : "Incoming"}</div><div className="mt-1 text-sm font-bold text-white">{item}</div></div>)}</div></div>
        </div>
        <div className="flex min-h-0 flex-col gap-3">
          ...
          <div className="rounded-[22px] border border-amber-300/25 bg-amber-500/10 p-3"><div className="text-[10px] uppercase tracking-[0.22em] text-amber-100">Audience prompt live</div><div className="mt-1 text-sm font-black uppercase tracking-[0.16em] text-white">{tapCoach.prompt}</div><div className="mt-1 text-xs text-amber-50/85">{tapCoach.detail}</div></div>
      ...
      <div className="mk3-demo-focus-ring" style={hostFocusFrame}><span>{hostFocusFrame.label}</span></div>
      <div className="mk3-demo-sim-cursor is-host" style={hostCursorStyle}><span>{activeScene.host.search ? "type" : "click"}</span></div>
    </div>
  </div>
);

export const DemoTvRoomShell = ({ roomCode, activeScene, activeLyric, nextLyric, totalConnectedLabel, reactionItems, triviaRows, roomEnergy, tvFocusFrame, formatClockLabel, tapCoach }) => (
  <div className="relative h-full overflow-hidden rounded-[28px] border border-white/12 bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.16),transparent_32%),linear-gradient(180deg,#16071a_0%,#090d18_42%,#04070f_100%)] text-white shadow-[0_30px_70px_rgba(0,0,0,0.42)]">
    <div className="relative grid h-full grid-cols-[0.92fr_1.34fr_0.96fr] gap-3 p-3">
      ...
      <div className="mk3-demo-focus-ring" style={tvFocusFrame}><span>{tvFocusFrame.label}</span></div>
    </div>
  </div>
);

export const DemoAudienceRoomShell = ({ roomCode, activeScene, activeActionIndex, activeFeedIndex, audienceTapStyle, audienceFocusFrame, tapCoach }) => (
  <div className="relative h-full overflow-hidden rounded-[24px] bg-gradient-to-b from-[#16061d] via-[#0d1121] to-[#050914] text-white">
    ...
    <div className="relative flex h-full flex-col">
      ...
      <div className="flex-1 overflow-hidden p-4">
        <div className="space-y-3">
          <div className="overflow-hidden rounded-2xl border border-indigo-500/30 bg-indigo-900/70 shadow-lg backdrop-blur-md">
            <div className="border-b border-white/10 bg-gradient-to-r from-black/70 via-black/30 to-black/70 px-4 py-3"><div className="text-[12px] uppercase tracking-widest text-indigo-300 font-bold mb-1">NOW PERFORMING</div><div className="font-bold text-xl leading-none truncate text-white">{activeScene.singer.name}</div><div className="text-sm text-indigo-200 italic truncate">{activeScene.tv.title}</div><div className="mt-3 flex flex-wrap gap-2"><button type="button" tabIndex={-1} className="rounded-full border border-fuchsia-400/40 bg-fuchsia-500/15 px-3 py-1 text-xs font-bold text-fuchsia-200"><i className="fa-solid fa-tv mr-1" />Video</button><button type="button" tabIndex={-1} className="rounded-full border border-cyan-400/40 bg-cyan-500/15 px-3 py-1 text-xs font-bold text-cyan-200"><i className="fa-solid fa-align-left mr-1" />Lyrics</button><button type="button" tabIndex={-1} className="rounded-full border border-pink-400/30 bg-pink-500/20 px-3 py-1 text-xs font-bold text-pink-200">+ Tight 15</button></div></div>
            <div className="p-4">
              <div className="rounded-2xl border border-cyan-300/35 bg-cyan-500/12 px-3 py-3"><div className="text-[10px] uppercase tracking-[0.2em] text-cyan-100">{tapCoach.title}</div><div className="mt-1 text-base font-black uppercase tracking-[0.14em] text-white">{tapCoach.prompt}</div><div className="mt-1 text-xs leading-relaxed text-cyan-50/85">{tapCoach.detail}</div></div>
      ...
      <div className="mk3-demo-focus-ring is-phone" style={audienceFocusFrame}><span>{audienceFocusFrame.label}</span></div>
